...
server.conf - LRRD-server configuration file
...
"server.conf" is the configuration file for lrrd-server, lrrd-update, lrrd-graph and lrrd-nagios.
...
The format of the file is simple. A minimal configuration looks something like:
	...
	tmpldir         /etc/lrrd/templates/

	[machine1.your.dom]
		address localhost

The default location of server.conf is @@CONFDIR@@/server.conf. If your placement
deviates from this norm, use the "--config E<lt>fileE<gt>"-option when running
the lrrd-* programs.

LRRD-update will expand all node-entries in this file, and save them to
