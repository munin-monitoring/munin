...
class GetCollectionUsage {
   ...
   {   this.memtype = memtype;
    ...
    public String[] GC() throws IOException, MalformedObjectNameException {
        ...
        int i = 0;
            GCresult[i++] =gcmbeans.get(memtype).getCollectionUsage().getCommitted()+ "";
            GCresult[i++] = gcmbeans.get(memtype).getCollectionUsage().getInit()+"";	
	    GCresult[i++] = gcmbeans.get(memtype).getCollectionUsage().getMax()+"";
	    GCresult[i++] = gcmbeans.get(memtype).getCollectionUsage().getUsed()+"";
