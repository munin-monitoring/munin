...
sub _start_tls {
    ...
    $self->{logger}->debug("[TLS] Enabling TLS.") if $self->{DEBUG};
...
sub _tls_verify_callback {
    ...
    return sub {
        ...
        if ($ok) {
            ...
            $self->{logger}->debug("[TLS] Verified certificate.") if $self->{DEBUG};
            return 1;           # accept
        }
        
        if (!($tls_verified->{"verify"})) {
            $self->{logger}->debug("[TLS] Certificate failed verification, but we aren't verifying.") if $self->{DEBUG};
            $tls_verified->{"verified"} = 1;
            return 1;
        }

        if ($tls_verified->{"level"} > $tls_verified->{"required_depth"}) {
            $self->{logger}->error("[TLS] Certificate verification failed at depth ".$tls_verified->{"level"}.".");
...
sub _log_cipher_list {
    ...
    $cipher_list .= '\n';
    $self->{logger}->debug("[TLS] Available cipher list: $cipher_list.") if $self->{DEBUG};
}
...
sub _set_ssleay_file_descriptors {
    ...
    my $err = &Net::SSLeay::print_errs("");
    if (defined $err and length $err) {
        $self->{logger}->warning("[TLS] Warning in set_rfd: $err");
    }
    Net::SSLeay::set_wfd($self->{tls_session}, $self->{write_fd});
    $err = &Net::SSLeay::print_errs("");
    if (defined $err and length $err) {
        $self->{logger}->warning("[TLS] Warning in set_wfd: $err");
    }
}
...
sub _accept_or_connect {
    ...
    $self->{logger}->debug("[TLS] Accept/Connect: $self->{private_key_loaded}, " . $self->_use_key_if_present()) if $self->{DEBUG};
    my $res;
    ...
    $self->{logger}->debug("[TLS] Done Accept/Connect") if $self->{DEBUG};
    ...
    {
	$self->{logger}->debug("[TLS] TLS enabled.") if $self->{DEBUG};
	$self->{logger}->debug("[TLS] Cipher `" . Net::SSLeay::get_cipher($self->{tls_session}) . "'.") if $self->{DEBUG};
	$self->{logger}->debug("[TLS] client cert: " . Net::SSLeay::dump_peer_certificate($self->{tls_session})) if $self->{DEBUG};
    }
}
...
sub read {
    ...
    my $err = &Net::SSLeay::print_errs("");
    if (defined $err and length $err) {
        $self->{logger}->warning("[TLS] Warning in read: $err");
        return;
    }
...
sub write {
    ...
    my $err = &Net::SSLeay::print_errs("");
    if (defined $err and length $err) {
        $self->{logger}->warning("[TLS] Warning in write: $err");
        return 0;
    }
