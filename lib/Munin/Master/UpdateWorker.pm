...
sub uw_handle_config {
	...
	# Pass service_id so uw_handle_fetch can compute CDEF thresholds
	my $update_rate = $service_attr{"update_rate"} || 300;
	my $timestamp;
	$timestamp = $self->uw_handle_fetch($plugin, $now, $update_rate, \@fetch_data, $service_id) if (@fetch_data);
...
#
# WHY PASS service_id?
# After writing raw DS values to RRD, we compute CDEF thresholds.
# This requires knowing which service we're in to query for CDEFs.
# The service_id is computed in uw_handle_config and passed here.
sub uw_handle_fetch {
	my ($self, $plugin, $now, $update_rate, $data, $service_id) = @_;
	...
	# --------------------------------------------------------------------
    # Compute CDEF thresholds after all raw DS values are written.
    #
    # WHY HERE (after the loop, before commit)?
    # - All raw DS values must be in RRD before xport reads them
    # - xport auto-flushes rrdcached (FETCH implies flush)
    # - Compute once per service, not per DS
    # - Still inside transaction, so alarm writes are atomic
    # --------------------------------------------------------------------
    if ($service_id) {
        $self->_compute_cdef_thresholds($service_id, $plugin);
    }
...
# ========================================================================
# CDEF THRESHOLD COMPUTATION
# ========================================================================
#
# WHY COMPUTE CDEFs HERE (at update time) INSTEAD OF AT LIMITS TIME?
#
# 1. SEPARATION OF CONCERNS:
#    - Update phase: fetch data, store in RRD, compute derived values
#    - Limits phase: read pre-computed alarms, send notifications only
#    This makes limits a pure read operation, much simpler and faster.
#
# 2. RRDCACHED COMPATIBILITY:
#    RRDs::xport (which uses FETCH internally) automatically flushes
#    rrdcached before reading. From rrd_client.c:
...
#    The FETCH response includes FlushVersion, meaning rrdcached flushed.
#    So we can safely call xport immediately after update.
#
# 3. BATCH EFFICIENCY:
#    Multiple CDEFs can be computed in a SINGLE RRDs::xport call.
#    Just add more CDEF: and XPORT: lines. One I/O per service.
#
# 4. COST PROPORTIONAL TO USAGE:
#    Only services with CDEF thresholds pay the xport cost.
#    Most CDEFs have no thresholds, so no overhead for them.
#
# ========================================================================

sub _compute_cdef_thresholds {
    my ($self, $service_id, $service_name) = @_;
    my $dbh = $self->{dbh};
    my $dbdir = $config->{dbdir};

    # --------------------------------------------------------------------
    # STEP 1: Find all DS in this service that have BOTH:
    #   - A cdef attribute (it's a computed/virtual field)
    #   - A warning or critical attribute (someone wants thresholds)
    #
    # We only compute CDEFs when thresholds are set. No thresholds = skip.
    # This is the key optimization: most CDEFs have no thresholds.
    # --------------------------------------------------------------------
    my $sth_cdef = $dbh->prepare(q{
        SELECT d.id, d.name, d.type,
               cdef.value AS cdef_expr,
               warn.value AS warn_val,
               crit.value AS crit_val
        FROM ds d
        INNER JOIN ds_attr cdef ON cdef.id = d.id AND cdef.name = 'cdef'
        LEFT JOIN ds_attr warn ON warn.id = d.id AND warn.name = 'warning'
        LEFT JOIN ds_attr crit ON crit.id = d.id AND crit.name = 'critical'
        WHERE d.service_id = ?
          AND cdef.value IS NOT NULL AND cdef.value != ''
          AND (warn.value IS NOT NULL AND warn.value != ''
               OR crit.value IS NOT NULL AND crit.value != '')
    });
    $sth_cdef->execute($service_id);

    my @cdef_ds;  # [{id, name, cdef_expr, warn_val, crit_val}, ...]
    while (my $row = $sth_cdef->fetchrow_hashref) {
        push @cdef_ds, $row;
    }
    return unless @cdef_ds;  # Nothing to compute

    DEBUG "[DEBUG] _compute_cdef_thresholds: $service_name has " . scalar(@cdef_ds) . " CDEFs with thresholds";

    # --------------------------------------------------------------------
    # STEP 2: Get RRD file paths for all DS in this service.
    #
    # We need the RRD file to call RRDs::xport. Each DS has its own RRD
    # file, but all DS in a service share the same rrdcached socket.
    # --------------------------------------------------------------------
    my %rrd_files;  # ds_name => rrd_file_path
    ...
    $sth_rrd->execute($service_id);
    while (my ($name, $file) = $sth_rrd->fetchrow_array) {
        $rrd_files{$name} = File::Spec->catfile($dbdir, $file);
    }

    # --------------------------------------------------------------------
    # STEP 3: Build xport arguments.
    #
    # For each CDEF DS, we need:
    #   - DEF for each source DS referenced in the cdef expression
    #   - CDEF for the computed field
    #   - XPORT to get the result
    #
    # RRDs::xport can compute multiple CDEFs in one call, so we batch
    # all CDEFs for this service into a single xport invocation.
    # --------------------------------------------------------------------
    my @xport_args = (
        '--start', 'now-3600',  # Look back 1 hour for fresh data
        '--end',   'now+120',   # Include current minute
        '--step',  '60',        # 1-minute resolution
    );

    # Track which DEFs we've already added (avoid duplicates)
    my %seen_defs;

    # Map from cdef_expr -> ds_name for result extraction
    my %cdef_map;  # result_name => {ds_name, warn_val, crit_val}

    for my $ds (@cdef_ds) {
        my $name = $ds->{name};
        my $cdef = $ds->{cdef_expr};
        my $rrd = $rrd_files{$name};

        unless (defined $rrd && -f $rrd) {
            WARN "[WARN] RRD file not found for CDEF $name: $rrd";
            next;
        }

        # ----------------------------------------------------------------
        # Parse the CDEF expression to find referenced DS names.
        #
        # CDEF expressions are RPN (Reverse Polish Notation):
        #   "in,out,+" means: push in, push out, add them
        #   "in,100,*,in,out,+,/" means: in*100/(in+out)
        #
        # We need to identify which tokens are DS names (variables)
        # vs operators/constants. DS names are alphanumeric; operators
        # are symbols like +, -, *, /, GT, IF, etc.
        #
        # This is a heuristic but works for 99% of real CDEFs.
        # ----------------------------------------------------------------
        my @tokens = split(/,/, $cdef);
        my @source_ds;
        for my $tok (@tokens) {
            # Skip if already seen or if it looks like a number/operator
            next if $seen_defs{$tok};
            next if $tok =~ /^[-+]?\d/;        # Number
            next if $tok =~ /^[<>=!+*\/\-]/;   # Single-char operator
            next if $tok =~ /^(GT|GE|LT|LE|EQ|NE|IF|MIN|MAX|LIMIT|DUP|POP|EXC|SIN|COS|LOG|EXP|FLOOR|CEIL|ABS|UNKN|INF|NEGINF|PREV|NOW|TIME|LTIME)$/i;
            # Looks like a DS name - add DEF if we have its RRD
            if (defined $rrd_files{$tok}) {
                push @xport_args, "DEF:${tok}=$rrd_files{$tok}:$tok:AVERAGE" unless $seen_defs{$tok}++;
                push @source_ds, $tok;
            }
        }

        # Add CDEF and XPORT for this computed field
        # Use a unique result name to avoid collisions
        my $result_name = "cdef_${name}";
        push @xport_args, "CDEF:${result_name}=$cdef";
        push @xport_args, "XPORT:${result_name}";

        $cdef_map{$result_name} = {
            ds_id    => $ds->{id},
            ds_name  => $name,
            warn_val => $ds->{warn_val},
            crit_val => $ds->{crit_val},
        };
    }

    return unless %cdef_map;  # Nothing to export

    # --------------------------------------------------------------------
    # STEP 4: Execute xport to compute all CDEF values.
    #
    # WHY THIS WORKS WITH RRDCACHED:
    # RRDs::xport calls rrdc_fetch() internally, which sends FETCH
    # to rrdcached. The FETCH command automatically flushes the file
    # before returning data (see rrd_client.c:1953 - FlushVersion check).
    # So even though we just wrote data via RRDs::update (which goes
    # to rrdcached memory), xport will see the fresh data.
    # --------------------------------------------------------------------
    DEBUG "[DEBUG] RRDs::xport(@xport_args)";
    my ($start, $end, $step, $nb_vars, $columns, $values) = RRDs::xport(@xport_args);

    if (my $err = RRDs::error) {
        ERROR "[ERROR] RRDs::xport failed for $service_name: $err";
        return;
    }

    # --------------------------------------------------------------------
    # STEP 5: Extract the LAST non-NaN value for each CDEF.
    #
    # xport returns an array of time series. We want the most recent
    # valid value for threshold comparison. Walk backwards to find it.
    # --------------------------------------------------------------------
    my %result_values;  # result_name => numeric_value
    for my $i (0 .. $#$columns) {
        my $col_name = $columns->[$i];
        next unless exists $cdef_map{$col_name};

        # Walk backwards to find last non-NaN value
        for my $j (reverse 0 .. $#$values) {
            my $val = $values->[$j][$i];
            if (defined $val) {
    ...
    # --------------------------------------------------------------------
    # STEP 6: Evaluate thresholds and write to state table.
    #
    # Store BOTH the computed value AND the alarm state.
    #
    # WHY STORE THE VALUE?
    # - CDEFs have no raw data in RRD; the computed value IS their value
    # - state.last_value is what Limits.pm reads for display/notifications
    # - Web UI can show the computed value without re-running xport
    # - Debugging: see exactly what value triggered an alarm
    #
    # WHY NOT A SEPARATE TABLE?
    # - state table already has last_value, alarm columns
    # - Limits.pm already reads from state; no schema change needed
    # - One row per DS keeps it simple
    # --------------------------------------------------------------------
    my $sth_update = $dbh->prepare(q{
        UPDATE state
        SET last_value = ?, last_epoch = ?, alarm = ?
        WHERE id = ? AND type = 'ds'
    });

    my $now = time();

    for my $result_name (keys %result_values) {
        my $meta = $cdef_map{$result_name};
        my $val = $result_values{$result_name};
        my $ds_id = $meta->{ds_id};
        my $warn = $meta->{warn_val};
        my $crit = $meta->{crit_val};

        # Evaluate: critical first (higher priority), then warning
        my $alarm = 'ok';

        if (defined $crit && $crit ne '') {
            my ($crit_lo, $crit_hi) = _parse_threshold_range($crit);
        ...
        if ($alarm eq 'ok' && defined $warn && $warn ne '') {
            my ($warn_lo, $warn_hi) = _parse_threshold_range($warn);
        ...
        DEBUG "[DEBUG] CDEF threshold: $meta->{ds_name} = sprintf('%.2f', $val), alarm=$alarm";
        $sth_update->execute(sprintf('%.6f', $val), $now, $alarm, $ds_id);
    }
}

# Parse threshold range like "80" or "80:90" or ":90" or "80:"
# Returns ($low, $high) where undef means no bound
sub _parse_threshold_range {
    my ($str) = @_;
    return (undef, undef) unless defined $str && $str =~ /\S/;

    if ($str =~ /^\s*([-+]?[\d.]*):([-+]?[\d.]*)\s*$/) {
        my $lo = length($1) ? $1 + 0 : undef;
        my $hi = length($2) ? $2 + 0 : undef;
        return ($lo, $hi);
    } elsif ($str =~ /^\s*([-+]?[\d.]+)\s*$/) {
        return (undef, $1 + 0);  # Single value = upper bound
    }
